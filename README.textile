...
Rah_flat comes with build in support for few essential content types: styles, forms, pages, preferences and sections. See the "templates":https://github.com/gocom/rah_flat/tree/master/templates directory on GitHub for an example how this all works.
...
If you want to exclude certain content type from importing, just don't create a directory for it. No directory, and rah_flat will leave the database alone when it comes to that content type.
...
Specifies path to the root templates directory containing all the content-type specific directories. This path is relative to your 'textpattern' installation directory. For instance a path @../templates@ would point a directory located in the same directory as your _textpattern_ directory and the main _index.php_ file.
